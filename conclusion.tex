...
scalable with HYPRE multigrid preconditioners \cite{hypre_hypre_2022} from PETSc. Moltres passes
drift closure terms computed from the $S_N$ transport solution as transport corrections
...
algorithm automatically truncates transport correction parameters near the $S_N$ problem subdomain
boundaries to discard inaccurate correction parameters and preserve smooth neutron
...
neutron diffusion methods to be assessed against. Discrepancies arose from the eight neutron
energy group structure, evidenced by differences in $k_\text{eff}$ and flux distributions between
OpenMC simulations on continuous energy (OpenMC-CE) and multigroup (OpenMC-MG) modes. Otherwise,
the multigroup $S_8$ method showed agreed well with OpenMC-MG. Although $k_\text{eff}$ estimates
...
OpenMC-CE). In comparison, the neutron diffusion method fared worse at 5.5 \%
...
which were marked improvements over the neutron diffusion method error estimates ranging from
569 pcm to 1484 pcm. The hybrid method also showed improvements in the absolute mean
...
boundaries is likely the primary source of discrepancy, and known neutron leakage
correction techniques, such as group-wise or matrix albedo boundary conditions, could reduce it.
