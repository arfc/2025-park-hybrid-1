...
a neutron diffusion solver iteratively coupled through transport correction and boundary flux
...
the \gls{MOOSE} framework. Implementing the $S_N$ method in Moltres enables native interfacing
...
We apply the \gls{SUPG} stabilization technique \cite{brooks_streamline_1982} for numerical
stability in near-void regions, first applied to \gls{SAAF} $S_N$ equations by Wang et al.\
\cite{wang_diffusion_2014}, to obtain a modified formulation as follows
%
\begin{multline}
...
convergence. Drift correction terms are prominent in existing literature as transport
...
diffusion coefficient values are limited to a maximum value of $D_g=2.5$ cm. Excessively large
$D_g$ values occurring in near-void regions cause numerical instabilities and computational
performance degradation. The limits have no impact on the converged solution because the diffusion
term containing $D_g$ is canceled out by the transport correction terms as the solution converges.
...
full reactor core models are computationally intensive and limited in their applicability to
...
the $S_N$ subproblem.
To limit the coverage of $V_1$ to the control rod region and its
immediate vicinity, $V_1$ should be sufficiently smaller than $V_0$ yet large enough to capture
flux anisotropies caused by the control rod. As a consequence, the boundaries $\partial V_1$ 
lie well within $V_0$. We obtain the best estimate for boundary fluxes for the $S_N$ solver by
applying the $P_1$ approximation to evaluate the neutron angular flux along
...
The $S_N$ subsolver boundary conditions will cause deviations
...
The $S_N$ subsolver generates accurate transport correction values ($\vec{D}_g$) within $V_1$
up to a few centimeters range of $\partial V_1$.

For accurate flux estimates with the hybrid method, $V_1$ must be large enough to
provide accurate transport corrections near the highly-absorbing control rod while simultaneously
accommodating for inaccurate transport correction values near $\partial V_1$. Before each
neutron diffusion calculation, the solver must identify and discard inaccurate transport correction
values. We will refer to the region near $\partial V_1$ in which the
...
where the components change signs). At these locations, the flux is approximately isotropic along
the axes corresponding to the components because value of $\vec{D}_g$ represents how much
correction a standard neutron diffusion model requires. This criterion also preserves the
smoothness of the neutron flux and flux gradient.
%
\begin{figure}[b]
	\centering
...
Here, we use a case from the 1-D test cases to be covered in Section \ref{sec:msre} to illustrate
the buffer region and the behavior of $\vec{D}_g$. This case, Case 3b, is a 1-D heterogeneous slab problem
consisting of control rod, air, fuel-graphite repeating lattice, and reactor vessel regions as
shown in Figure \ref{fig:3b-geometry}. The fuel regions constitute neutron-multiplying media
containing $^\text{235}$U fissile material. The material compositions of each region are from the
\gls{MSRE} reference design and the start-up fuel composition \cite{fratoni_molten_2020}.
...
iteration scheme in the \texttt{MultiApps} system for coupling multiple individual solvers
(referred to as apps) requires users to specify a primary app for the final convergence check
