...
where the components change signs). This criterion are two reasons. Firstly, at
points where the $\vec{D}_g$ components are zero, the flux is approximately isotropic along the
corresponding axes because $\vec{D}_g$ represents the correction a
standard neutron diffusion model requires. Secondly, this choice preserves the
neutron flux and flux gradient smoothness.
%
\begin{figure}[b]
	\centering
