...
We limited the diffusion coefficients ($D_g$) to a maximum default value of 2.5 cm to avoid having
excessively large values in near-void regions causing numerical instabilities and computational
performance degradation.
The limits have no impact on the converged solution because the diffusion
...
In order to reduce the computational cost of the high-order transport step, we propose reducing the
...
Henceforth, we will refer to the $S_N$ calculation on the correction
...
The $S_N$ subsolver requires boundary sources along $\partial V_1$ derived from the
neutron diffusion solution of the previous iteration. We can obtain estimates for boundary fluxes
...
%
\begin{equation}
D_{g,j}=0\ \left(j=x,y,\mbox{ or }z\right)
\end{equation}
%
evaluated along the corresponding Cartesian axes. This preserves flux gradient smoothness across
the interface between corrected and uncorrected flux solutions. Section \ref{sec:flux-drift-dist}
discusses how we implemented an adaptive algorithm for determining buffer region size in the
context of a 1-D test cases.

In our investigations, the buffer region provided a two- to three-fold reduction in the total
...
(Eq.\ \ref{eq:drift}), flux magnitude updates in the later iterations do not impact it. In a
...
The iterative algorithm for the hybrid $S_N$-diffusion method is as follows:
%
\begin{enumerate}
  \item Start with an initial neutron diffusion calculation in $V_0$ using the standard neutron
    diffusion method.
  \item Pass the neutron diffusion flux solution along $\partial V_1$ to the $S_N$ subsolver to
    evaluate $S_N$ boundary source distribution using Eqs.\ \ref{eq:boundary-source} and
    \ref{eq:p1-boundary}.
  ...
  \item Repeat Steps 2-5 until flux solutions meet convergence criteria.
\end{enumerate}
%
Figure \ref{fig:algorithm} shows a visual flowchart of the hybrid method algorithm.
The hybrid method iterations terminate when the initial residual evaluated at the start of
a $S_N$ with updated boundary source from the previous diffusion solution remains below a
...
\begin{figure}[p]
  ...
  \begin{tikzpicture}
    ...
    \node (2) [process, below of=1, yshift=-1.8cm]
      {\textbf{Compute incident flux boundary conditions along\\$\bm{\partial V_1}$ for $\bm{S_N}$ subsolver}};
    ...
    \node (7) [object, below of=6, yshift = -2cm]
      {\textbf{Output latest\\ $\bm{\phi_g}$ and $\bm{k}$ solution}};
    \draw [arrow] (1) -- node[anchor=east] {$\bm{k^{(0)}, \phi^{(0)}_g, J^{(0)}_g}$} (2);
    \draw [arrow] (2) -- node[anchor=east] {$\bm{\Psi^{\textbf{inc}}_{g,d}}$ \textbf{along} $\bm{\partial V_1}$} (3);
    \draw [arrow] (3) -- node[anchor=east] {$\bm{\phi^{tr,(i)}_g, J^{tr,(i)}_g}$} (4);
    \draw [arrow] (4) -- node[anchor=east] {$\bm{\vec{D}^{(i)}_g}$} (5);
    \draw [arrow] (5) -- node[anchor=east] {$\bm{k^{(i)}, \phi^{(i)}_g, J^{(i)}_g}$} (6);
...
framework \cite{giudicelli30MOOSEEnabling2024}, which facilitates multiphysics coupling with different
...
%tolerance thresholds $\epsilon_\text{tol}$. The iterations terminate when the initial
