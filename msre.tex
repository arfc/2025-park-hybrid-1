...
precursor data.
All simulations in this work approximated 2nd-order-accurate scattering cross sections using
...
This choice balances accuracy and computational expense appropriately for this work when
...
Reference \gls{MSRE} specifications \cite{robertson_msre_1965, fratoni_molten_2020} provide the
...
of approximately 55000 pcm in the 1-D models, effectively halving the $k_\text{eff}$. The worth
...
We ran mesh convergence tests on Case 3b for the neutron diffusion and $S_N$ neutron
transport methods.
...
They deviate from OpenMC-CE by approximately 350 pcm for Cases 2a and 3a, but fall within
...
reactivity discrepancies to the eight neutron energy group structure, the only notable difference
between OpenMC-CE and the multigroup methods.

The neutron diffusion and hybrid methods agree closely for Cases 2a and 3a, which
exclude the control rod region. This shows the hybrid method provides $k_\text{eff}$ estimates
similar to the neutron diffusion method in 1-D simulations without highly neutron-absorbing
...
effect of introducing the control rod region, evidenced by the -1500 pcm and -1150 pcm
...
Figure \ref{fig:1d-worth} shows the percentage
difference in rod worths for Cases 2 and 3 of OpenMC-MG, $S_8$, neutron diffusion, and hybrid
methods relative to OpenMC-CE. We calculated each method's rod worth estimates by taking reactivity
difference between the `rod out' (a) and `rod in' (b) configurations of Cases 2
...
The neutron diffusion methods are clear outliers with rod worth estimates in excess
of 8\%. The remaining methods cluster around 2.5\% and 3\% percentage difference for Cases
2 and 3.

Since the hybrid method relies on transport corrections derived from the $S_N$ method,
$S_8$ rod worth estimates serve as the
reference point for hybrid method verification. Errors from the multigroup
approximation affect both hybrid and $S_N$ simulations similarly. Nevertheless, the hybrid
...
Further discussion in this section focuses on Cases 3a and 3b, which closely resemble the
actual \gls{MSRE} reactor with a control rod withdrawn and inserted, respectively.
%
...
%groups.
%
For a fair evaluation of the deterministic multigroup methods without multigroup approximation
distortions, we compared their flux distributions with the OpenMC-MG flux
...
exhibits large flux deviations in groups 1, 2, 7, and 8 near $x=0$ cm.
The hybrid method significant improves neutron flux distributions compared to the neutron diffusion
method, shown by generally smaller per-group flux deviations particularly near $x=0$ cm.
...
Figure \ref{fig:3b-drift} shows the distributions of the drift correction parameter in each of the
neutron energy group for Case 3b. The hybrid method selectively applies the $S_N$ method to improve
neutronic modeling within the subregions. We computed
the reference distributions (orange data points) in both plots from the standard $S_8$ methods's
reference flux solutions.
...
generate flux corrections. Minimizing the correction subregion size and the $S_N$ subproblem
is essential for the hybrid method's computational competitiveness in time-dependent full-core
...
at 5 cm-intervals. All subregion sizes place the subregion boundary more than one neutron mean free
...
pcm for Case 3b. The $k_\text{eff}$ values did not
converge towards the $S_8$ method's $k_\text{eff}$ estimate. This behavior
implies that neutron leakage error at the outer boundary remains a notable source of
...
worth estimates do not notably change with varying correction subregion
sizes. The rod worth estimates for all investigated correction subregion sizes are within
0.2\% of the $S_8$ method. This indicates limiting the correction subregion size to save
computational cost on expensive $S_N$ calculations has a negligible impact on rod worth
estimates.
...
reproduce rod worth estimates as long as the correction subregion size is consistent between
$k$-eigenvalue simulations used for rod worth calculations. The
correction subregion size must also be large enough to capture the highly neutron-absorptive
...
for neutron transport methods that do not
...
before the solver meets the fixed point convergence tolerance.
...
$\epsilon_\text{tol}$ to $10^{-5}$ balances accuracy and computational cost effectively because the
$k_\text{eff}$ error is less than 0.01 pcm, and further tightening the tolerance would increase the
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=.8\columnwidth]{quarter-core-geom}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=.8\columnwidth]{full-core-geom}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.45\columnwidth]{full-core-closeup}
...
change in sign of the error to -816 pcm. The errors for OpenMC-MG and the hybrid method are
positive in sign and reduced to 446 pcm and 760 pcm, respectively. Consequently, when taking the
...
\end{align}
%
The total fission rate includes fission rate contributions from the peripheral fuel salt regions.
...
1/8th slices of the core because the distributions are symmetrical about the $y=x$ line, excluding
statistical variances from OpenMC-CE. The midpoints of fuel salt channels to form the
...
center as expected of a highly neutron-absorbing material. The rod further exacerbated fission rate
errors for the neutron diffusion method with errors of 1\% or more at the core center and
periphery. On the other hand, the fission rate errors for the hybrid method largely stayed below
1\%.
...
method. The absolute maximum percentage error from the neutron diffusion method increased to
17.44\%
as compared to 3.08\% for the hybrid method in the ``Rod'' case. These
results demonstrate the hybrid method provides improved fission rate distribution accuracy in
near near-void and highly neutron-absorbing regions.
...
increase with the number of rods inserted. The mean percentage error from the hybrid method is
constant at 0.43\% while the maximum percentage error increases marginally from 1.45\% to 2.52\%
when all three rods are inserted. These results demonstrate the hybrid method is
...
the angular dependence in the neutron angular flux. Table \ref{table:2d-sn-convergence} shows the
$k_\text{eff}$ estimates from the hybrid method with the $S_6$, $S_8$, $S_{10}$, and $S_{12}$
methods for the 2-D full-core with no control rods inserted.
The $k_\text{eff}$ varied by less than 1 pcm, thereby indicating the angular discretization
order used by the hybrid method had little impact on the integral reactor parameters.
...
insufficient angular resolution. The artifacts diminish with increased angular
...
orders. Larger fission cross sections heavily weight $k_\text{eff}$ towards the later (slower)
energy groups, which clarifies why Table \ref{table:2d-sn-convergence} shows little
...
Further investigations into the angular discretization level may be necessary when applying the
hybrid method to reactors with more air-filled or highly-heterogeneous regions, such as
...
\begin{figure}[p]
  ...
  region with $S_6$, $S_8$, $S_{10}$, \& $S_{12}$. Ray effects are barely visible in groups
